...
function normalizeEnvironment(explicit, inferred) {
    return explicit ?? inferred ?? 'development';
}
function normalizeApi(api, envApi, mode) {
    const fallbackBaseUrl = mode === 'api' || mode === 'hybrid' ? DEFAULT_API_BASE_URL : undefined;
    if (!api && !envApi && !fallbackBaseUrl)
        return undefined;
...
export function networkProvider(config) {
