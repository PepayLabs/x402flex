...
import {
  ...
  FlexRouterInput,
  FlexWitnessStruct,
  CreateFlexIntentParams,
  FlexIntentResult,
  PaymentSettledEvent,
  FlexPaymentIntentJSON,
} from './types.js';
import {
  derivePaymentId,
  deriveResourceId,
...
export function getFlexSchemeId(scheme: string): string {
  return resolveFlexSchemeId(scheme);
}

export function buildFlexResponse(input: FlexResponseInput): FlexResponse {
  ...
  const referenceFallback = () =>
    normalizeReference(input.referenceId ?? `flex_${Date.now()}`);

  const accepts = input.accepts.map((accept) =>
    buildFlexAcceptOption(accept, {
...
export function createFlexIntent(params: CreateFlexIntentParams): FlexIntentResult {
  ...
  const witness: FlexWitnessStruct = {
    schemeId,
    intentHash,
    payer: params.payer ? ethers.getAddress(params.payer) : ethers.ZeroAddress,
...
function buildFlexAcceptOption(
  option: FlexAcceptInput,
  context: {
    referenceId?: string;
  ...
  const reference = normalizeReference(
    option.referenceId ?? context.referenceId ?? context.referenceFallback()
  );
...
export function decodePaymentSettledEvent(log: Log | { data: string; topics: string[]; blockNumber?: number; transactionHash?: string }): PaymentSettledEvent {
  const decoded = PAYMENT_REGISTRY_INTERFACE.decodeEventLog('PaymentSettledV2', log.data, log.topics);
  const referenceData = decoded.referenceData as string;
