...
import { ethers, type ContractRunner, type BigNumberish } from 'ethers';
import {
  PaymentRequest,
  PaymentRequestURI,
  PaymentReceipt,
...
import { FlexPaymentIntentStruct } from './types.js';

// X402FlexRegistry ABI (synced via export script)
export const PAYMENT_REGISTRY_ABI = X402FlexRegistry__factory.abi as ethers.InterfaceAbi;

type RouterPaymentOptions = TransactionOptions & {
 ...
 */
export async function createPaymentRequestWithQR(
  options: Parameters<typeof createPaymentRequest>[0]
): Promise<PaymentRequestURI> {
...
export async function buildPaymentTransaction(
  ...
  routerAddressOrUnused?: string,
  options?: RouterPaymentOptions
  ...
  if (mode === 'direct') {
    ...
    const router = X402FlexRouter__factory.connect(routerAddress, provider as ContractRunner);
    ...
    const txOverrides: Record<string, unknown> = { ...(options || {}) };
    ...
    const intent: FlexPaymentIntentStruct = {
      paymentId,
      merchant: request.recipient,
      token: tokenAddress,
      amount: tokenAmount,
...
export async function canPay(params: {
  ...
  const registry = X402FlexRegistry__factory.connect(
    ethers.getAddress(registryAddress),
    provider as ContractRunner
  );
  const [ok, reason] = await registry.canPay(
...
export async function verifyPayment(
  ...
  const minConf = minConfirmations || DEFAULT_CONFIRMATION_BLOCKS[chainId as keyof typeof DEFAULT_CONFIRMATION_BLOCKS] || 1;
  ...
  if (paymentSettledEvent) {
    const parsedLog = PAYMENT_REGISTRY_INTERFACE.parseLog({
      topics: paymentSettledEvent.topics as string[],
...
export async function isPaymentSettled(
  ...
  const registry = X402FlexRegistry__factory.connect(paymentRegistry, provider as ContractRunner);
