...
import { PROTOCOL_VERSION, MAX_REFERENCE_LENGTH, MIN_NONCE_LENGTH, TOKEN_DECIMALS } from './constants.js';
const FLEX_SCHEME_ALIASES = {
...
export function resolveFlexSchemeId(scheme) {
    if (!scheme)
 ...
 */
export function generateNonce(length = MIN_NONCE_LENGTH) {
 ...
 */
export function generateReferenceId(prefix = 'order') {
    const timestamp = Date.now();
    const random = generateNonce(8);
    return `${prefix}_${timestamp}_${random}`;
}
const PAYMENT_INTENT_TYPEHASH = ethers.keccak256(ethers.toUtf8Bytes('PaymentIntent(bytes32 paymentId,address merchant,address token,uint256 amount,uint256 deadline,address payer,bytes32 resourceId,bytes32 referenceHash,bytes32 nonce)'));
const PAYMENT_ID_TYPEHASH = ethers.keccak256(ethers.toUtf8Bytes('PaymentId(address token,uint256 amount,uint256 deadline,bytes32 resourceId,bytes32 referenceHash,bytes32 nonce)'));
const FLEX_WITNESS_TYPEHASH = ethers.keccak256(ethers.toUtf8Bytes('FlexWitness(bytes32 schemeId,bytes32 intentHash,address payer,bytes32 salt)'));
export function hashPaymentIntent(intent) {
...
export function hashFlexWitness(witness) {
    const abi = ethers.AbiCoder.defaultAbiCoder();
    return ethers.keccak256(abi.encode(['bytes32', 'bytes32', 'bytes32', 'address', 'bytes32'], [
...
export function deriveEip3009Nonce(params) {
    ...
    return ethers.solidityPackedKeccak256(['string', 'bytes32', 'address', 'uint256'], ['X402Flex', intentHash, router, chainId]);
}
 ...
 */
export function encodePaymentURI(request) {
    ...
    if (request.referenceId)
        params.append('referenceId', request.referenceId);
    if (request.label)
        params.append('label', request.label);
    if (request.message)
        params.append('message', request.message);
    if (request.expires)
        params.append('expires', request.expires.toString());
    if (request.metadata) {
        const metadataStr = ethers.encodeBase64(ethers.toUtf8Bytes(JSON.stringify(request.metadata)));
...
export function decodePaymentURI(uri) {
    ...
    if (params.has('currency'))
        request.currency = params.get('currency');
    if (params.has('token'))
        request.token = params.get('token');
    if (params.has('referenceId'))
        request.referenceId = params.get('referenceId');
    if (params.has('label'))
        request.label = params.get('label');
    if (params.has('message'))
        request.message = params.get('message');
    if (params.has('expires'))
        request.expires = parseInt(params.get('expires'));
    if (params.has('metadata')) {
        try {
            const metadataStr = ethers.toUtf8String(ethers.decodeBase64(params.get('metadata')));
 ...
 */
export function parseTokenAmount(amount, tokenSymbol) {
    const decimals = TOKEN_DECIMALS[tokenSymbol] || 18;
 ...
 */
export function calculateReferenceHash(reference) {
...
export function deriveResourceId(params) {
    ...
    const abi = ethers.AbiCoder.defaultAbiCoder();
    const encoded = abi.encode(['address', 'string', 'address', 'uint256', 'uint64', 'bytes32'], [
...
export function derivePaymentId(params) {
    const abi = ethers.AbiCoder.defaultAbiCoder();
    const encoded = abi.encode(['bytes32', 'address', 'uint256', 'uint256', 'bytes32', 'bytes32', 'bytes32'], [
        PAYMENT_ID_TYPEHASH,
